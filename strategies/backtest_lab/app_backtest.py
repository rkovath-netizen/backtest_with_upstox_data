    ...
    if st.button("🚀 Run Historical Backtest", use_container_width=True):
        ...
        try:
            ...
            if results_df.empty:
                ...
                # ==========================================
                # FILE EXPORT & NAMING CONVENTION
                # ==========================================
                timestamp_str = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
                strategy_name = "EMA_RSI_DynamicExits"
                file_name = f"{strategy_name}_{timestamp_str}.csv"
                
                # 1. Save locally to server's output directory
                output_dir = "dataoutput"
                os.makedirs(output_dir, exist_ok=True)
                local_path = os.path.join(output_dir, file_name)
                results_df.to_csv(local_path, index=False)
                st.success(f"File generated on server: `{local_path}`")
                
                # 2. Provide instant download button for user
