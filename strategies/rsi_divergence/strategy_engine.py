...
def process_rsi_divergence(symbols, start_date, end_date, upstox_token, timeframe="15min", 
    ...
    # Map UI Timeframe strings to Pandas Resample strings
    tf_map = {
        "3 Minutes": "3min",
        "5 Minutes": "5min",
        "15 Minutes": "15min",
    ...
    for sym_idx, symbol in enumerate(symbols):
        ...
        # Fetch 1m Futures Data
        df_1m = fetch_continuous_futures_candles(symbol, warmup_start, end_dt, upstox_token, log_func=lambda x: None)
        if df_1m.empty:
            log_func(f"❌ Failed to fetch futures data for {symbol}. Skipping.")
            continue

        log_func(f"📊 Resampling to {timeframe} and calculating Divergences...")
        
        # Resample to the requested Macro Timeframe
        ...
        entries = []
        left_bars = 3  # Lookback to confirm a pivot
        right_bars = 2 # Lookforward to confirm a pivot
        ...
        for j in range(left_bars + right_bars, len(df_tf) - 1):
            ...
            # --- PIVOT LOW LOGIC (Bullish Divergence) ---
            ...
            if center_low == min(window_lows):
                ...
                # Check for Divergence against the last pivot low
                if last_pl_idx != -1 and (center_idx - last_pl_idx) <= 50:
                    ...
                    if extreme_ok and ((use_regular and is_reg_bull) or (use_hidden and is_hid_bull)):
                        ...
                        entries.append({
                            'time': df_tf['timestamp'].iloc[j + 1], # Execute on Next Bar Open
            ...
            # --- PIVOT HIGH LOGIC (Bearish Divergence) ---
        ...
        # 🚨 Trade Management is ALWAYS processed on 3-Minute data for precise SL/Target execution
        ...
        for idx, trade in enumerate(entries):
            ...
            log_func(f"⚡ [{symbol}] Executing {signal_name} at {entry_time} (Futures: {entry_price})")
            ...
            # Synchronize 3m Data for exit processing
